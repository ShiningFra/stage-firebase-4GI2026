        ...
        <div className="flex flex-col items-center text">
            ...
            <div className="w-full lg:w-[37.688rem] py-[1.5rem] lg:py-[3rem] relative">
              <div className='flex sm:space-x-5 space-x-3'>
                <div className="w-auto inline-block px-2 py-2 bg-[#FE9261] rounded-2xl absolute top-[-0.1rem] left-0">
                  <h2 className="text-white font-bold text-center text">{t('etiquette')}</h2>
                </div>
                <div
                    className="w-auto inline-block sm:px-2 flex border-2 flex-row py-2 rounded-2xl absolute top-[-0.1rem] left-[10rem]">
              ...
              <div className="space-y-4">
                <button
                  ...
                  className="w-full sm:w-auto bg-blue-600 text-white px-8 py-3 rounded-md hover:bg-blue-700 transition-colors duration-300 flex items-center justify-center"
                >
            ...
            <div className="flex flex-col items-center w-full">
              ...
              <div className="flex flex-wrap w-full justify-between px-4">
                {benefits.map((item, index) => (
                    <div key={index} className="flex items-center flex-col justify-center w-[22rem]">
                      {item.icon && <item.icon className="h-20 w-20 mr-2 text-[#2D3A96]"/>}
                      <h3 className="title text-justify  my-[1rem]">{item.title}</h3>
                      <p className="text text-justify  w-full">{item.description}</p>
                    </div>
                ))}
              </div>
            </div>
            <div className="flex flex-col items-center mt-[1rem] w-full px-4">
              ...
              <div className="flex flex-wrap w-full justify-between">
                {options.map((item, index) => (
                    <div key={index} className="flex items-center flex-col justify-center my-3 w-[20rem] md:w-[30rem]">
                      <Image src={item.img} alt={item.title} className='w-45'/>
                      <h3 className="title  text-justify font-bold my-[2rem]">{item.title}</h3>
                      <p className="text text-justify">{item.description}</p>
                      <Link href="#"
                            className="text-[#FE9261] hover:text-primary text mt-[1rem] underline" >{item.link}</Link>
                    </div>
                ))}
              </div>
            </div>
          ...
          <div className="container mx-auto bg-white my-[1rem] rounded-2xl shadow-xl flex flex-col md:flex-row justify-center items-center  px-4 py-8">
            <div className="flex flex-col justify-center  w-full md:w-[50rem] gap-4">
              <h3 className="bigtitle text-[#2D3A96] font-bold">{t('P4title')}</h3>
